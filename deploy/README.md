...
| Secrets from SOPS | ✅ **validated** | `secrets.sh k8s` → `supabase-env`, `kestra-env`, `supabase-db-initdb` Secrets; `bootstrap.sh` also bridges `kms-auth` and (SSH-CA) `pki-oidc` |
| SSH-CA env (`decision-024`/`decision-028`) | ✅ **wired** | **functions** Deployment: `PKI_BASE_URL` + `PKI_FLEET_TOKENS` (zone→fleet-token JSON map, `ssh-ca` host-cert signing). **backend** Deployment: `PKI_BASE_URL` / `PKI_OIDC_TOKEN_URL` / `PKI_OIDC_CLIENT_ID` inline + `PKI_OIDC_CLIENT_SECRET` from the `pki-oidc` Secret (Keycloak service account that creates domain zones, `task-080`). Values in `secrets/supabase.enc.env` (fleet tokens) + `secrets/iotgw-ui-backend.enc.env` (OIDC) |
