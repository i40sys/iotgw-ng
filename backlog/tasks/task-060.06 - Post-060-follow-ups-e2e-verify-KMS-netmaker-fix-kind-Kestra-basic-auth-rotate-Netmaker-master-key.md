...
3. Rotate the Netmaker master key: `NBMtSWau…GtjH5` (redacted; full value in git history — rotate per task-053) was hardcoded in the (now-removed) device_*/network_* Kestra playbooks and lived in i40sys/iotgw-kestra history — treat as compromised. Rotate at the Netmaker dashboard, update secrets/netmaker.enc.env (SOPS) + secrets/supabase.enc.env if it carries NETMAKER_*, re-render, and update the kind Secret. (decision-014 rotation runbook.)
