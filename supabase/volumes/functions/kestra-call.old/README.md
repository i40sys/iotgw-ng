...
- ✅ Calls Kestra flows via multipart/form-data (matching your curl example)
- ✅ Supports dynamic namespace and flow ID
- ✅ Passes input parameters as form fields
...
```bash
curl -v -X POST -H 'Content-Type: multipart/form-data' -F 'user=Rick Astley' 'http://wsl.ymbihq.local:8080/api/v1/main/executions/tutorial/hello-world'
```

But provides a more structured JSON API interface while maintaining compatibility with Kestra's multipart/form-data requirement.
