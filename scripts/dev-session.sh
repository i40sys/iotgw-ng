#!/usr/bin/env bash
#
# dev-session.sh — attach-or-create for the iotgw-ng dev stack.
#
# The dev stack (mprocs: iotgw-ui app + backend + backlog) lives in a tmux
# session named after this repo. Running this script is ALWAYS safe and
# idempotent:
#
...
# would silently start a second one on shifted ports (vite auto-bumps 5173->5174,
# so the OLD stack keeps 5173 while the new UI drifts — the classic duplicate).
#
# Other projects run their own mprocs (e.g. pki-manager on 52080/6430,
# process-links on 5173) — this script never matches by process name, only by
# this repo's session name and by the cwd of whoever holds our ports.
#
# Usage:
...
# Fixed ports this stack binds — clustered around 52173 so it coexists with the
...
# True if anything is LISTENing on $1, even when its pid isn't visible to us.
...
scan_ports() {
  ...
  for p in "${PORTS[@]}"; do
    pid="$(port_pid "$p")"
    if [ -z "$pid" ]; then
      # No visible pid: if the port is still held, it's another user's process
      # we can neither inspect nor kill — treat as foreign so `up` refuses
      # cleanly instead of letting the server fail to bind.
      port_listening "$p" && THEIRS+=("$p:?:? (other user)")
      continue
    fi
    cwd="$(pid_cwd "$pid")"
...
up)
  ...
  # "Stdin is not a tty" — even when launched by an agent. mprocs reads
  # ./mprocs.yaml from the session cwd (repo root) — app + backend + backlog.
  # COREPACK_ENABLE_DOWNLOAD_PROMPT=0: `pnpm dev` otherwise blocks each proc on
  # Corepack's interactive "download pnpm? [Y/n]" prompt inside the pane.
  tmux new-session -d -s "$SESSION" -c "$REPO" -x 200 -y 50 \
    "COREPACK_ENABLE_DOWNLOAD_PROMPT=0 mprocs; printf '\n[dev stack stopped] press Enter to close this tmux session...'; read -r _"
